# -*- coding: utf-8 -*-
"""
RiskFailSafeObserver extends TbotOrderEvent to monitor the portfolio's daily unrealized P&L
and closes all positions if the P&L drops below the defined threshold (e.g., 1% loss).
It also includes a test mode to automatically close the "NVDA" position 120 seconds after it's filled.
"""
...
class RiskFailSafeObserver(TbotOrderEvent):
    def __init__(self, ibsyn: IB, orderdb, errordb, contract_pnl=None, loss_threshold: float = -1.0, test_mode: int = 0):
        ...
        :param orderdb: Instance of TbotOrderDB for handling order-related database operations.
        :param errordb: Instance of TbotErrorDB for handling error-related database operations.
        :param contract_pnl: List of contract PnL events to track.
        ...
        """
        super().__init__(ibsyn, orderdb, errordb, contract_pnl)
        ...
        # Subscribe to relevant IB events (inherited from TbotOrderEvent)
        self.install_event_hdlrs()

    def on_update_portfolio(self, item):
        """
        Override to add risk management functionality.
        Monitor portfolio updates and calculate daily P&L.
        """
        super().on_update_portfolio(item)  # Call parent method to handle portfolio update

        # Calculate the combined unrealized P&L for all positions and trigger closure if needed
        self.check_pnl()

    def on_pnl_single_event(self, pnl: PnLSingle):
        """
        Override to track PnL and perform risk mitigation if threshold is breached.
        """
        super().on_pnl_single_event(pnl)  # Call parent method to handle PnL event
        self.check_pnl()
    ...
    def on_order_status_event(self, trade):
        ...
        """
        super().on_order_status(trade)  # Call parent method to handle order status update
